...
void loadShaders(){
    ...
    floorShader->setFragOutputLocation("G_output", 0);
    floorShader->link();

    MVPLocation = floorShader->getUniformLocation("MVP");
    ViewLocation = floorShader->getUniformLocation("View");
    ModelLocation = floorShader->getUniformLocation("Model");
    floorShader->setMVPFunction([=](const Mat4& M, const Mat4& V, const Mat4& P){
        glUniformMatrix4fv(MVPLocation, 1, GL_FALSE, glm::value_ptr(P*V*M));
        glUniformMatrix4fv(ViewLocation, 1, GL_FALSE, glm::value_ptr(V));
